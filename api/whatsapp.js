...
async function insert(table, row) {
    ...
    await fetch(SUPA + '/rest/v1/' + table, { method: 'POST', headers: { apikey: key, Authorization: 'Bearer ' + key, 'Content-Type': 'application/json', Prefer: 'return=minimal' }, body: JSON.stringify(row) });
}
async function execAction(p) {
    const today = new Date().toISOString().slice(0, 10);
    if (p.action === 'add_task') { await insert('tasks', { text: p.text, cat: p.cat || 'pessoal', prio: p.prio || 'media', due: p.due || null, done: false }); return 'Tarefa adicionada: ' + p.text; }
...
export default async function handler(req, res) {
    ...
    try {
          ...
          return reply(res, 'Erro ao processar: ' + String(e).slice(0, 80));
    }
}
